...
const formatMenu = (menu) => {
  ...
  const normalized = typeof menu === 'string' ? menu.toLowerCase() : '';
...
export const generateInviteCard = async ({ payload, eventConfig }) => {
  ...
  const names = `${payload.primaryGuest.firstName} ${payload.primaryGuest.lastName}`.trim();
  ...
  ctx.fillText(`${eventConfig?.venue || ''} - ${dateText}`, centerX, cursorY);
